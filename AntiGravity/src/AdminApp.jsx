...
export default function AdminApp() {
    ...
    const renderQrGenView = () => {
        const [selectedPodId, setSelectedPodId] = useState('');
        const [qrValue, setQrValue] = useState('');

        const generateQrCode = () => {
            if (selectedPodId) {
                // In a real app, this would be a dynamic URL pointing to the pod's booking page
                setQrValue(`https://urbannaps.com/pod/${selectedPodId}`);
            } else {
                setQrValue('');
            }
        };
        ...
        return (
            <div className="p-8 animate-fade-in max-w-3xl">
                ...
                <p className="text-sm text-slate-500 mb-8">Generate and print QR codes for your pods to enable quick access for users.</p>

                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-6">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">Select a Pod</label>
                        <select
                            value={selectedPodId}
                    ...
                    {qrValue && (
                        <div className="pt-6 border-t border-slate-100 flex flex-col items-center justify-center space-y-4">
                            <p className="text-sm font-medium text-slate-700">Scan this QR code to access the pod:</p>
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans flex text-slate-900">
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {currentView === 'qr-gen' && renderQrGenView()}
            </div>
        </div>
    );
}
